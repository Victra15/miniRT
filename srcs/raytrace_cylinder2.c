...
/*   Updated: 2022/11/14 18:24:12 by yolee            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_vec3	get_normal_vec(t_cylinder *cylinder, t_vec3 hit_point)
{
	...
	if (v_inner(cen_to_hp, cylinder->orient) == cylinder->sca.height / 2
		|| v_inner(cen_to_hp, cylinder->orient) == -cylinder->sca.height / 2)
	...
	t_vec3 hit_point)
{
	...
	light_diffuse = v_inner(
			v_unit(v_diff(hit_point, data->light.light_point)), norm);
